...
rendering of the images to:

- Add an overlay shown when the mouse enters pictures with the ALT label

- Add a fancybox (lightbox) for the image whenever it got resized, but not when
  the image was not resized (does not make sense to show a lightbox with the
  very same image...

- Do not do those changes for small inline images (say smaller than 100px width)

This is just a matter of playing with TypoScript, something you should love
anyway as a TYPO3 integrator. Please note that the extension comes with a
directory containing `TypoScript snippets`_.

.. tip::

   Need some ideas of what is feasible? Please have a look at a
   `post of mine in Google+`_.


Available modes for the plugin
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

When integrating the plugin with TypoScript, additional modes of operations are
available. These are:
...
This extension registers an additional content object to let you access
meta-information from your documentation. Example:
